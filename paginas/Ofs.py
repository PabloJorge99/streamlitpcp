...
# Garante que a coluna 'Final' é datetime e remove valores nulos
...
# Verifica os nomes corretos das colunas
colunas_disponiveis = df_filtrado.columns
colunas_necessarias = ['status', 'Plano', 'Final', 'Sub-g','Ordem F']

# Faz um mapeamento se for necessário, ou adapta os nomes conforme os reais:
# Supondo que "Sub-G" e "Ordem" sejam os nomes reais (substitua se forem diferentes)
...
# Pie chart for Tipo_Lote distribution inside columns
...
with col1:
    ...
    # Display the pie chart for Tipo_Lote
...
with col2:
    ...
    fig_sub_g = px.pie(sub_g_count, values=sub_g_count.values, names=sub_g_count.index, 
                       title="Distribuição de OFs por Sub-g", 
    ...
    # Display the pie chart for Sub-g
